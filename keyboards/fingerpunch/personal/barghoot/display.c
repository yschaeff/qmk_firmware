...
uint8_t qmk_lv_get_cpi(void) {
    uint8_t cpi = 0;
    cpi = (fp_snipe_keycode_get() || fp_snipe_layer_get()) ? fp_get_cpi_value_from_mode(FP_SNIPING_MODE) : fp_get_cpi_value_from_mode(FP_POINTING_MODE);
    return cpi;
}
...
void lvgl_event_triggers(void) {
    ...
    uint8_t curr_cpi   = (fp_snipe_keycode_get() || fp_snipe_layer_get()) ? fp_get_cpi_value_from_mode(FP_SNIPING_MODE) : fp_get_cpi_value_from_mode(FP_POINTING_MODE);
